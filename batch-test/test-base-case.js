...
const testBaseCase = async () => {
  ...
  console.log(`   PARALLEL_COUNT: ${JSON.stringify(testParams.PARALLEL_COUNT)}`);
  ...
  console.log(`   REPEAT_COUNT: ${JSON.stringify(testParams.REPEAT_COUNT)} (type: ${typeof testParams.REPEAT_COUNT})`);
  console.log('');
  ...
  for (const fileCount of fileCounts) {
    ...
    const files = Array.from({ length: fileCount }, (_, i) => ({ id: i + 1, name: `file${i + 1}.txt` }));
    
    const batches = chunkArray(files, batchSize);
    
    console.log(`   Result: ${batches.length} batch${batches.length === 1 ? '' : 'es'} created`);
  ...
  try {
    ...
    delete require.cache[require.resolve('../controllers/data/dropbox-vault/batchConfig.js')];
    const { getCurrentConfig } = require('../controllers/data/dropbox-vault/batchConfig.js');
    const config = getCurrentConfig();
    
    console.log(`   Generated BATCH_SIZE in config: ${config.BATCH_SIZE}`);
    console.log(`   Generated PARALLEL_COUNT in config: ${config.PARALLEL_COUNT}`);
    console.log(`   Generated WAVE_DELAY in config: ${config.WAVE_DELAY}`);
    console.log(`   Generated BATCH_STAGGER in config: ${config.BATCH_STAGGER}`);
