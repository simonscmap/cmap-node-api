...
const testRepeatCount = () => {
  ...
  console.log(
    `   Input: REPEAT_COUNT = ${
      testConfigInteger.REPEAT_COUNT
    } (type: ${typeof testConfigInteger.REPEAT_COUNT})`,
  );
  ...
  console.log(
    `   Expected combinations: ${
      2 * 2 * 2 * 2 * 1
    } = 16 (excluding REPEAT_COUNT)`,
  );

  // Check if REPEAT_COUNT is excluded from combinations
  const hasRepeatCount = combinationsInteger.some((combo) =>
    Object.prototype.hasOwnProperty.call(combo, 'REPEAT_COUNT'),
  );
  console.log(`   REPEAT_COUNT included in combinations: ${hasRepeatCount}`);

  if (combinationsInteger.length === 16 && !hasRepeatCount) {
    console.log(
      '   ✅ CORRECT - Integer REPEAT_COUNT properly excluded from combinations',
    );
  } else {
  ...
  console.log(
    `   Input: REPEAT_COUNT = ${JSON.stringify(
      testConfigArray.REPEAT_COUNT,
    )} (type: ${typeof testConfigArray.REPEAT_COUNT})`,
  );
  ...
  // Check if REPEAT_COUNT is excluded from combinations
  const hasRepeatCountArray = combinationsArray.some((combo) =>
    Object.prototype.hasOwnProperty.call(combo, 'REPEAT_COUNT'),
  );
  console.log(
    `   REPEAT_COUNT included in combinations: ${hasRepeatCountArray}`,
  );

  if (combinationsArray.length === 16 && !hasRepeatCountArray) {
    console.log(
      '   ✅ CORRECT - Array REPEAT_COUNT properly excluded from combinations',
    );
  } else {
  ...
  console.log(
    `   From integer config: ${repeatCountFromInteger} (type: ${typeof repeatCountFromInteger})`,
  );
  console.log(
    `   From array config: ${repeatCountFromArray} (type: ${typeof repeatCountFromArray})`,
  );

  if (repeatCountFromInteger === 3 && repeatCountFromArray === 3) {
    console.log(
      '   ✅ CORRECT - Both integer and array formats extract correctly',
    );
  } else {
