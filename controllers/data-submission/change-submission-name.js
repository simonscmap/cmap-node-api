...
let log = initializeLogger("controllers/data-submission/change-submission-name");
...
const copyFiles = async ({ prevPath, newPath }) => {
  ...
  const [checkErr, checkResult, metadata] = await runWithh5Retries (checkFn, checkPred);
...
const deleteFolder = async (folderPath) => {
  ...
  try {
    ...
    log.error ('error deleting folder', arg);
    error = e;
  }
