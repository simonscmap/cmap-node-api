...
exports.csv = async (req, res, next) => {
    ...
    req.cmapApiCallDetails.ignore = true;

    request.on('recordset', recordset => {
        console.log('hello')
        transformer = new CSVStream({objectMode: true}, recordset);
        transformer.on('error', () => console.log('error'));
        ...
        if(!res.headersSent){
            ...
            request.pipe(transformer).pipe(res);
        }
    })
