...
const getGriddedWithHourlyQuery = (variableData) => {
  ...
  const depthConraint =
    Has_Depth && targetDepthRange.length === 2
...
const constructQuery = (variableData, latCount) => {
  ...
  const shouldRestrictSpatialParams =
    queryType === 'sp' && latCount > oneMillionOneHundredThousand;
...
const sampleVis = async (req, res, next) => {
  ...
  log.info('preparing sample visualization', {
    variableId: varId,
    datasetShortName: shortname,
  });

  // get dataset info
  const [e, data] = await getDatasetVisualizableVariables(
    { shortname },
    requestId,
  );
  if (e) {
  ...
  if (!selectedVariable) {
    log.error('could not find variable in dataset variables', {
      variables,
      varId,
    });
