...
const safelyGetResponseJson = async (resp) => {
  ...
  try {
    ...
    moduleLogger.error('error parsing response for popular datasets', {
      error: e,
...
const fetchPopularDatasetsFromValidationAPI = async () => {
  ...
  const tableNames = Object.entries(data.data).map(([, tablename]) => ({
    tablename,
  }));
...
const fetchPopularDatasetsWithCache = async () =>
  await cacheAsync('POPULAR_DATASETS', fetchPopularDatasetsFromValidationAPI, {
...
module.exports.recentDatasets = async (req, res, next) => {
  ...
  const [err, responseData] =
    await fetchRecentDatasetsFromValidationAPI(user_id);
...
module.exports.recommendedDatasets = async (req, res, next) => {
  const { user_id } = req.query;
  const [err, responseData] =
    await fetchRecommendedDatasetsFromValidationAPI(user_id);
