...
const assembleTrajectories = (cIds = [], tPts = [], options) => {
  ...
  ids.forEach ((cId) => {
    if (scalingFactor > 1 && !options.downSample) {
      Object.assign(trajectoryMap[cId], analyzeTrajectory(trajectoryMap[cId], options));
    } else {
      // scaling factor is > 1 AND downSample is indicated
      const downsampledData = downsampleTrajectory (trajectoryMap[cId], scalingFactor, options);
      trajectoryMap[cId] = Object.assign(downsampledData, analyzeTrajectory(downsampledData, options));
    }
  });
