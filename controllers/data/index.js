...
const toSetArray = (data) => {
  if (!data || !Array.isArray(data)) {
    moduleLogger.error(
      'toSetArray received incorrect arg; expecting arrray ',
      data,
    );
    return [];
  }
...
const fetchSprocQuery = async (reqId, spExecutionQuery, argSet) => {
  ...
  try {
    ...
    log.error('error fetching sproc statement', {
      error: e,
      query: spExecutionQuery,
    });
  ...
  if (
    ...
    log.info('sproc call returned query', {
      argSet,
      result: result.recordset[0].query,
    });
    ...
    log.error('error fetching sproc statement: no result', {
      query: spExecutionQuery,
      result,
    });
    return [true, null, 'error fetching sproc statement'];
...
const customQuery = async (req, res, next) => {
  ...
  if (isSproc(req.query.query)) {
    let uspDataRetrievingNames =
      await fetchDataRetrievalProcedureNamesWithCache();
    ...
    if (
      uspDataRetrievingNames
        .map((name) => name.toLowerCase())
        .includes(sprocName)
    ) {
      let spExecutionQuery = `${req.query.query}, 1`;

      log.info('fetching query for stored procedure', {
        sproc: spExecutionQuery,
      });

      let [error, queryToRun] = await fetchSprocQuery(
        req.requestId,
        spExecutionQuery,
        req.query,
      );
...
const queryModification = async (req, res, next) => {
  ...
  if (isSproc(req.query.query)) {
    ...
    // query is not executing a sproc
...
const storedProcedure = async (req, res, next) => {
  ...
  if (argSet.spName === 'uspVariableMetadata') {
    ...
    let [error, q, message] = await fetchSprocQuery(
      req.requestId,
      spExecutionQuery,
      req.query,
    );
...
const datasetFeatures = async (req, res, next) => {
  ...
  if (
    ...
    result.recordsets.length === 2
  ) {
    let featureMap = transformFeatureResults(result.recordsets, log);
    res.json(featureMap);
    next();
  } else {
    log.error('incomplete response while fetching dataset features', {
      result,
    });
    res.status(500).send('incomplete response');
    return next(
      new Error('incomplete response while fetching dataset features'),
    );
  }
};
...
const fetchTrajectoryPointCounts = async (log) => {
  ...
  const query =
    'select Cruise_ID, count(Cruise_ID) points from tblCruise_Trajectory group by Cruise_ID';
