...
const detectBulkDownloadSizeError = (error, log = moduleLogger) => {
  ...
  if (error.code === 'EREQUEST') {
    ...
    if (errorNum === 701 && message.includes('insufficient system memory')) {
      log.info('detected SQL Server memory exhaustion error', {
        errorNum,
        message,
      });
      return {
        ...
        statusCode: 413,
        message:
          'Dataset too large for download. Please apply filters to reduce the size.',
      };
    }
    ...
    if (
      errorNum === 8645 &&
      message.includes('timeout occurred while waiting for memory')
    ) {
      log.info('detected SQL Server memory timeout error', {
        errorNum,
        message,
      });
      return {
    ...
    if (errorNum === 511 && message.includes('Cannot create a row of size')) {
      log.info('detected SQL Server row size limit error', {
        errorNum,
        message,
      });
      return {
  ...
  if (error.code === 'ECONNRESET' || error.code === 'ETIMEDOUT') {
    ...
    if (
      ...
      message.includes('Timeout: Request failed')
    ) {
      log.info('detected connection error during large transfer', {
        code: error.code,
        message,
      });
      return {
  ...
  log.debug('error not identified as size-related', {
    code: error.code,
    number: error.number,
  });
