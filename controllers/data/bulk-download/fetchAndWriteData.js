...
const { getCandidateList } = require("../../../utility/router/queryToDatabaseTarget");
...
const delegate = async (targetInfo, query, candidateLocations) => {
  let remainingCandidates = await onPremToDisk (targetInfo, query, candidateLocations);
...
const fetchAndWriteData = async (tempDir, shortName, reqId) => {
  ...
  try {
    ...
    log.error ('failed to write metadata to disk', { targetPath, shortName, error: e });
  ...
  const [tablesErr, tablesResp] = await directQuery(getTableNameQuery, getTableNameOpts, log)
  if (tablesErr) {
  ...
  const fetchAndWriteJobs = tables
    .map(async (tableName) =>
      await routeQuery(
