...
const getRowCountForQuery = async (tablename, constraints, dataset, requestId) => {
  ...
  // cluster result
  if (Array.isArray(countResult) && countResult.length === 1 && countResult[0].c) {
    return [null, countResult[0].c];
  }

  // on prem result
  if (typeof countResult === 'object' && Array.isArray(countResult.recordset) && countResult.recordset.length && countResult.recordset[0].c) {
