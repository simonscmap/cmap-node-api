...
const subscribeUserToDataset = async (userId, shortName, log = moduleLogger) => {
  // get dataset id
  const datasetId = await getDatasetId (shortName, log);

  if (!datasetId) {
    log.error ('failed to create subscription, could not find dataset id', { shortName });
  ...
  if (checkExistsResult.length) {
    log.info ('subscription already exists', { result: checkExistsResult, shortName: shortName });
    return [false, {
