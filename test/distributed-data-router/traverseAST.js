...
let qWithBackticks =
  'SELECT DISTINCT(Dataset_ID) FROM tblVariables WHERE LOWER(Table_Name)=LOWER(`tblAltimetry_REP_Signal`)';
...
test('can parse top n distinct ', (t) => {
  let [error, result] = transformTopQueryToLimit(
    `select TOP 2 distinct time from tblMyTable order by time desc`,
  );
  t.is(error, false);
  t.is(
    result,
    'SELECT DISTINCT time FROM tblMyTable ORDER BY time DESC LIMIT 2',
  );
});
