...
passport.use(
  ...
  new CustomStrategy(async (req, done) => {
    try {
      ...
      let checkTokenResult = await checkTokenRequest.query(
        `SELECT [Hash], [Times_Used] from [tblGuest_Tokens] WHERE ID = @id`
      );

      if (checkTokenResult.recordset[0].Times_Used > 9)
        return done(null, false);
      if (
        checkTokenResult.recordset[0].Hash !== guestTokenHashFromRequest(req) ||
  ...
  new JwtStrategy(jwtExtractorOpts, async function (req, jwtPayload, done) {
    ...
    try {
      let unsafeUser = new UnsafeUser(
        await UnsafeUser.getUserByID(jwtPayload.sub)
      );
      req.cmapApiCallDetails.userID = unsafeUser.id;
...
passport.use(
  new HeaderApiKeyStrategy(
    ...
    async function (apiKey, done, req) {
      try {
        let unsafeUser = new UnsafeUser(
          await UnsafeUser.getUserByApiKey(apiKey)
        );
