...
// String -> Boolean
const isJoin = S.test(S.regex('g')('\\sjoin\\s'));
...
// String -> String
let stripComments = S.map(removeSQLDashComments)(removeSQLBlockComments);
...
let replaceStarWithCols = (queryString, cols) => {
  ...
  let newQ = q.replace(selectStarRegex, replacer);
...
let expandStar = async (queryString) => {
  ...
  let { datasetTables } = compareTableAndDatasetLists(
    onPremTableList,
    datasetIds,
  );
  ...
  let datasetRecord = datasetIds.find(
    ({ Table_Name }) => Table_Name.toLowerCase() === tableName.toLowerCase(),
  );

  if (!datasetRecord) {
    return [
      `could not match record of dataset table ${tableName} to dataset id`,
    ];
  }
  ...
  if (columnNames.length === 0) {
    ...
    log.info('retrieved column names for select star', {
      datasetId,
      tableName,
    });
