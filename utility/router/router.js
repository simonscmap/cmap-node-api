...
async function delegateExecution(
  ...
  if (targetIsCluster && !queryIsExecutingSproc) {
    ...
    remainingCandidatesAfterFailure = await executeQueryOnPrem(
      req,
      res,
      next,
      query,
  ...
  if (
    Array.isArray(remainingCandidatesAfterFailure) &&
    remainingCandidatesAfterFailure.length > 0
  ) {
    log.info('allowing retry after failure', {
      remainingCandidatesAfterFailure,
    });
    return await delegateExecution(
      req,
      res,
      next,
      query,
  ...
  // no more retries
  log.trace(
    `execution returned null on attempt #${currentAttempt}, but no error nor retry; calling next`,
  );
  next();
}
