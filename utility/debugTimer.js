...
const debugTimer = (timerTitle, options = {}) => {
  ...
  const report = () => {
    ...
    let results = events_.map ((ev, i) => {
      const duration = (i !== events_.length - 1) ? events_[i + 1].time - ev.time : 0;
    ...
    if (options.mergeSimilar) {
      ...
      results = Array.from (keys).reduce((acc, currKey) => {
        ...
        const mergedEvents = matchingEvents.reduce ((mergeAcc, currEv) => {
          return {
            ...mergeAcc,
    ...
    formattedResults.push (formatEvent({ tag: 'TOTAL', share: 1, duration: elapsedTime }))

    if (typeof timerTitle === 'string') {
      console.log (`Profile results for ${timerTitle}`);
    }

    console.table (formattedResults.filter (ev => !['START', 'DONE'].includes(ev.tag) ));
  }
