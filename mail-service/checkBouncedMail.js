...
const updateRecipentsWithFailedDelivery = async (updates) => {
  ...
  log.info (`recipient email records updated`, { recipients: updates, rowsAffected: result.rowsAffected });
...
const monitor = async (emailIds = []) => {
  ...
  log.info ('mail monitor called', { emailIds, stillActiveIds: Array.from (activeIds) });
  ...
  if (err) {
    log.error ('error retrieving email records', { error: err, activeIds: Array.from (activeIds) });
    if (queued === false) {
  ...
  if (activeIds.size === 0) {
    log.info ('no active email ids to monitor', { activeIds: Array.from (activeIds), calledWith: emailIds });
    queued = false;
    return;
  } else {
  ...
  const newlyBouncedMails = matchingBouncedMails
        .filter((record) => {
          ...
          const bounceIsAfterLastAttempt = dayjs (bounceDateTime).isAfter (dayjs (lastAttempt));
  ...
  if (newlyBouncedMails.length) {
    log.info (`identified ${matchingBouncedMails.length} newly bounced mail(s)`, {
      activeIds: Array.from (activeIds),
      emailIds: matchingBouncedMails.map (r => r.emailId),
    });
    matchingBouncedMails.forEach (m => log.info ('newly bounced', m));
  } else {
    log.info ('did not identify any new bounced mail', { activeIds: Array.from (activeIds) });
  }

  // update recipient records with newly detected failures
  const [updateErr] = await updateRecipentsWithFailedDelivery (newlyBouncedMails);
